...
export default function LoginPage() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (s?.token && s.token !== "aalgorix-local-auth") {
          const defaultDest = (s.role === 'teacher' || s.role === 'admin') ? '/teacher' : '/student';
          router.replace(redirectTo && redirectTo !== '/dashboard' ? redirectTo : defaultDest);
        }
      }
    } catch {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { token, userId, username: mUser, firstname, lastname, fullname, role = 'student' } = json.data;
      ...
      // Respect explicit ?from= param; otherwise route by role
      const defaultDest = (role === 'teacher' || role === 'admin') ? '/teacher' : '/student';
      const dest =
        redirectTo && redirectTo !== '/student' && redirectTo !== '/dashboard'
          ? redirectTo
          : defaultDest;
